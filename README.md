[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/0AdnTben)
...
It may help to read some portions of [The Intel System Programming model Volume 3][intel_system_programming_model].
Specifically, chapters 4, 5, 7, 8, and 18 may be of interest.
...
* **Memory mapping**: mapping of the first 16 MB of physical memory, and the
  ...
  * All running processes can simply share the same global page tables.
* **Demand paging**: data is retrieved from the backing stores only when needed.
* **Backing store management**:
  * `read_bs`, `write_bs`: implemented and fully functional
* **Page replacement policy**: `SC`

Remember that, per the specification, page tables are allocated and removed on demand.
In other words, your system must not pre-allocate page tables aside from optionally
doing so for the global page tables representing physical memory.
...
Even with the design given to you this is a difficult project.
...
level) to run into problems. A suggested workflow is as follows:

1. Get Xinu running as-is with paging enabled .
    - This wll require setting up page directory and page tables for physical memory.
2. Implement xmmap without worrying about the backing store.
    - Implement page fault handler to map virtual memory to frames,
3. Get xmmap to persist to the approrpiate backing store.
    - This allows multiple processes to share data through a backing store.
4. Implement vcreate and private heaps.
5. Implement the second chance page replacement policy.

## 10. Grading Policy

* (70 pts) Submission can be compiled and pass the provided `testmain.c`
  successfully
* (30 pts) Six additional test cases (5 pts/testcase)
...
* This mapping is maintained inside the kernel.
  ...
  physical memory. The code currently defines a signature in [paging.h](h/paging.h)
  as the `bsm_tab`.
...
**Can multiple processes share backing stores?**

* Depending on how the backing store is reserved, they may. Multiple processes can xmmap to the same backing store, but they cannot xmmap to a backing store reserved for a process's virtual heap. Additionally, only one process may use a given backing store for its virtual heap.

**What data structures need to be stored where?**

- 2048-4095: Backing stores
- 1024-2047: Page directories, page tables, and page frames (the physical memory backing virtual pages)
- 0-1023: Existing Xinu structures such as:
    - Physical heap
    - Stacks
    - Inverted page table (frm_tab)
    - Backing store map (bsm_tab)

**What memory range should the virtual heap encompass?**

* Pages 4096-4351. We limit it to this to ensure that each virtual heap maps neatly to the owning process's reserved backing store.

**What should happen if we need to evict a frame that doesn't have a backing store reserved?**

* `xmmap` and `vcreate` should fail (return `SYSERR`) if there isn't a backing store available so all frames will always have a backing store.

**Does i386 invalidate TLB when changing page directory?**

* Yes, it does, specifically TLBs are invalidated by the MOV instruction to CR3. See 5.10.4.1 p.5-49 of Vol. 3A.

**Role of and interactions with the backing stores:**

In previous semesters, there has been a significant amount of confusion as to the purpose of the backing stores since they are physical memory that can be accessed as any othere physical memory.

Backing stores simulate disk space used by an operating system. This is done to avoid the need to perform I/O operations and simplify implementation, and would not be done in a real operating system. The backing store addresses can be accessed by processes using their exact physical addresses, but we ask that processes not directly access them. Instead Xinu will take care of writing data to and reading it from them using the `read_bs` and `write_bs` syscalls you will implement.

Processes will indirectly interact with backing stores either by using xmmap or when using their virtual heaps. Below are two examples of how these interactions may occur.

xmmap:
- Process A xmmaps backing store 1 to virtual page 4100
- Process B xmmaps backing store 1 to virtual page 4200
- A writes "abc" to vpage 4100 
    - This causes a page fault
    - The page fault handler reserves a frame now backing page 4100
    - The value is written
- B reads from virtual page 4200 
    - This causes a page fault
    - The page fault handler reserves a frame now backing page 4200 
        - This frame will not be the same frame backing A's vpage 4100
    - B reads the value "abc" that A wrote
- B modifies this value to "cba"
- A reads from page 4100 
    - A reads thee value "cba" that B wrote

When and how exactly the data is transmitted between processes A and B is flexible so long as they see the data as described above.

virtual heaps:
- Process A is created with a virtual heap 
    - Xinu reserves backing store 1 as swap space
- Process A writes some data to its virtual heap at page 4101 (vpage 5) 
    - This causes a page fault
    - The page fault handler reserves frame 6 (page 1030) now backing A's page 4101
- Process B is created with a virtual heap 
    - Xinu reserves backing store 2 as swap space
- B writes some data to its page 4096 (vpage 0) 
    - This causes a page fault
    - The frame table is full and the page fault handler determines frame 6 should be evicted based on the replacement policy 
        - Xinu writes the contents of frame 6 to page 2053 (bs1 page 5, corresponding to A's vpage 5)
        - Xinu updates page tables and TLB so that frame 6 now backs B's page 4096
- A reads data from its page 4101 
    - A page fault occurs
    - The page fault handler reserves frame 30 (page 1054) now backing A's vpage 4101 
        - In the interim other frames have been freed so no eviction is necessary
        - The data in bs 1 page 5 is loaded into frame 30
    - A successfully reads its previously written data

**There are a large number of function stubs throughout the code. What should I use them for?**

* Some of these function stubs are for the functions you will implement as part of the assignment. Others are simply meant as a starting point for helper functions. You are free to implement or ignore them as you see fit.